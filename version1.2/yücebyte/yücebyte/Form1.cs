...
namespace yücebyte
{
    public partial class Form1 : Form
    {
        ...
        private void CompressImage(Image sourceImage, int imageQuality, string savePath)
        {
            try
            {
            int yeniBoyutYuzdesi = Convert.ToInt32(lblBoyutDeger.Text);
                sourceImage  = resizeImage(sourceImage, new Size((sourceImage.Width* yeniBoyutYuzdesi)/100, (sourceImage.Height * yeniBoyutYuzdesi) / 100));
        ...
        private void trckBroKalite_Scroll(object sender, EventArgs e)
        {
            ...
            if(Convert.ToInt32(lblkaliteGoster.Text) <= 16) { lblkaliteGoster.ForeColor = Color.Red; }
            else if(Convert.ToInt32(lblkaliteGoster.Text) >= 70) { lblkaliteGoster.ForeColor = Color.Yellow; }
            else { lblkaliteGoster.ForeColor = Color.Green; }
        }

        private void trckbroBoyut_Scroll(object sender, EventArgs e)
        {
            lblBoyutDeger.Text = "" + Convert.ToInt32(trckbroBoyut.Value);
